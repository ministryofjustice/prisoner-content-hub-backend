...
class PrisonerHubWarmer extends WarmerPluginBase {
  ...
  private function warmPrisonHomePage(string $prison) {
    ...
    // Urgent Banners.
    $this->queueAsynchronousRequest($prison, "node/urgent_banner?include=field_more_info_page&fields%5Bnode--urgent_banner%5D=drupal_internal__nid%2Ctitle%2Ccreated%2Cchanged%2Cfield_more_info_page%2Cunpublish_on");
    // Updates.
    try {
  ...
  /**
   * Queues an asynchronous request for later processing.
   *
   * @param string $prison
   ...
   */
  protected function queueAsynchronousRequest(string $prison, string $path) {
    $this->queuedAsynchronousRequests[] = [$prison, $path];
  }
  ...
  protected function executeAllAsynchronousRequests() {
    ...
    foreach ($batchedRequests as $batch) {
      ...
      foreach ($batch as $request) {
        $promises[] = $this->warmJsonApiRequestAsync($request[0], $request[1]);
      }
