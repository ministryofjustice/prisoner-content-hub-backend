...
export function ensureE2ETaxonomyTerms(): void {
  ...
  const phpEval = [
    ...
    `$prisonHierarchy = ${phpSingleQuoted(JSON.stringify(defaultPrisonGroups))};`,
    '$prisonTermsByName = [];',
    '$seedPrisonTerm = function (string $name, int $parentTid = 0) use ($storage, &$prisonTermsByName): int {',
    '  $existing = $storage->loadByProperties(["vid" => "prisons", "name" => $name]);',
    '  if (empty($existing)) {',
    '    $term = \\Drupal\\taxonomy\\Entity\\Term::create([',
    '      "vid" => "prisons",',
    '      "name" => $name,',
    '      "parent" => [["target_id" => $parentTid]],',
    '    ]);',
    '    $term->save();',
    '    $prisonTermsByName[$name] = (int) $term->id();',
    '    return (int) $term->id();',
    '  }',
    '',
    '  $term = reset($existing);',
    '  if ($term->hasField("parent")) {',
    '    $currentParentIds = array_map(static fn ($item) => (int) $item->target_id, iterator_to_array($term->get("parent")));',
    '    if (!in_array($parentTid, $currentParentIds, true)) {',
    '      $term->set("parent", [["target_id" => $parentTid]]);',
    '      $term->save();',
    '    }',
    '  }',
    '  $prisonTermsByName[$name] = (int) $term->id();',
    '  return (int) $term->id();',
    '};',
    '$adultMaleTid = $seedPrisonTerm("Adult male", 0);',
    '$femaleTid = $seedPrisonTerm("Female", 0);',
    '$youthMaleTid = $seedPrisonTerm("Youth male", 0);',
    'foreach ($prisonHierarchy as $group) {',
    '  $parentTid = 0;',
    '  if ($group["name"] === "Adult male") {',
    '    $parentTid = $adultMaleTid;',
    '  }',
    '  elseif ($group["name"] === "Female") {',
    '    $parentTid = $femaleTid;',
    '  }',
    '  elseif ($group["name"] === "Youth male") {',
    '    $parentTid = $youthMaleTid;',
    '  }',
    '  foreach ($group["children"] as $prisonName) {',
    '    $seedPrisonTerm($prisonName, $parentTid);',
    '  }',
    '}',
    '$termIds = [];',
    ...
    '    if (!empty($adultMaleTid) && $term->hasField("field_prisons")) {',
    '      $term->set("field_prisons", [["target_id" => $adultMaleTid]]);',
    '    }',
    '    $term->save();',
    ...
    '    if (!empty($adultMaleTid) && $existingTerm->hasField("field_prisons") && $existingTerm->get("field_prisons")->isEmpty()) {',
    '      $existingTerm->set("field_prisons", [["target_id" => $adultMaleTid]]);',
    '      $existingTerm->save();',
    '    }',
    ...
    '    ]);',
    '    if (!empty($adultMaleTid) && $node->hasField("field_prisons")) {',
    '      $node->set("field_prisons", [["target_id" => $adultMaleTid]]);',
    '    }',
    '    $node->save();',
    '  }',
    '  else {',
    ...
    '    if (!empty($adultMaleTid) && $existingNode->hasField("field_prisons") && $existingNode->get("field_prisons")->isEmpty()) {',
    '      $existingNode->set("field_prisons", [["target_id" => $adultMaleTid]]);',
    '    }',
    '    $existingNode->save();',
    '  }',
    '}',
